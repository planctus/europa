  ...
  Drupal.behaviors.commissioner_filters = {
    attach: function(context) {
      ...
      if ('.filter-button'.length > 0) {
        $('.view-header .result-count', context).after('<div class="filter-button"><button class="btn btn-primary hidden-md hidden-lg" type="button" data-toggle="collapse" data-target="#' + blockFiltersId + '" aria-expanded="false" aria-controls="collapseFilters">' +
                                        refineText +
                                      '</button></div>');
      }
