...
Feature: Legacy links
  ...
  Scenario: Show relative legacy link on announcement teaser
    Given "Page" content:
      | title         | language | field_core_description | status | field_core_latest_visibility |
      | Content title | en       | Content description    | 1      | Enable                       |
    Given "Announcement" content:
      | title                      | language | status | field_core_type_content              | field_core_legacy_link | field_core_pages | field_announcement_type | field_announcement_location |
      | Announcement title on page | en       | 1      | Teaser linking to external resources | title - departments    | Content title    | press release           | Brussels                    |
    And I am logged in as a user with the "administrator" role
    And I go to "admin/content"
    And I follow "Content title"
    ...
    And I fill in "Moderation state" with "published"
    And I press "Save"
    Then I should see the link "Dutch announcement title on page" linking to "departments_nl"
    And I go to "admin/content"
    And I follow "Content title"
    Then I should see the link "Announcement title on page" linking to "departments_en"
